...
<main data-analytics-level1="body">
        ...
        <div class="pb-42 md:pb-38">
            <div class="flex flex-col gap-6 mx-auto md:max-w-208">
                <div class="top-18 sticky card-stack sm:top-20 md:top-25 lg:top-40 min-[1200px]:top-32!">
                    <h2 class="mb-10 text-navy-blue-300 text-center headline-1">Manfaat Email Khusus</h2>
                    <div data-analytics-level3="Build Legitimacy">
    ...
    <section
        ...
        <div x-data class="xl:top-20 flex flex-col gap-7.5 w-full md:gap-11.5 xl:sticky">
            <h2 class="text-navy-blue-300 headline-1 text-center">Cara Membuat <span class="sm:block">Email Khusus</span></h2>
            <div class="group relative mx-auto rounded-[10px] overflow-hidden cursor-pointer" @click="$store.videoModal.openModal('{{ config('onlinebersama.video_id.email_detail') }}')">
                <img class="size-full aspect-video object-cover max-w-140 rounded-[10px]" src="{{ asset('images/usecase-email.webp') }}" alt="">
                <div data-analytics-name="video image | How to Setup Custom Email" class="top-0 left-0 absolute bg-gradient-video size-full"></div>
                <button type="button" data-analytics-name="play button | How to Setup Custom Email" class="transition-all duration-300 ease-in-out cursor-pointer top-1/2 left-1/2 absolute -translate-x-1/2 -translate-y-1/2 group-hover:scale-110">
                    <x-icons.play-rounded width="78" height="78" fill="#fff" />
                </button>
            </div>
        </div>
