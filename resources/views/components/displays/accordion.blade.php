...
} }" :class="last ? 'border-y-3' : 'border-t-3'" class="border-t-3 border-mint-300">
    <div :class="open ? 'gap-4' : 'gap-0'" class="flex flex-col py-11">
        {{-- Title & Toggle --}}
        <div class="flex gap-4">
            <h2 :class="open ? 'py-3.5' : 'py-0'" class="subheadline-2 transition-all duration-300 ease-in-out w-full text-navy-blue-300">{{ $question }}</h2>
            <div class="flex items-center shrink-0">
                <button type="button" @click="toggleAccordion" class="cursor-pointer">
                    <img x-show="!open" src="{{ asset('icons/deep-plus.svg') }}">
                    <img x-show="open" x-cloak src="{{ asset('icons/deep-min.svg') }}">
                </button>
            </div>
        </div>
        {{-- Body/Content --}}
