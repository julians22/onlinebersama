<header x-data="{ dropdown: false, sidebar: false }" class="top-0 right-0 z-50 sticky w-full">
    {{-- Navigation Menu --}}
    <nav class="bg-gradient-nav">
        <div class="container-nav flex justify-between items-center gap-2.5 py-2 sm:py-3 lg:py-5">
            ...
            <div class="flex items-center lg:gap-x-4 xl:gap-x-10">
                <ul class="hidden lg:gap-x-4 lg:flex xl:gap-x-10">
                    <li class="flex items-center text-right"><a href="{{ route('index') }}#mengapa-com" class="nav-link">Mengapa .com?</a></li>
                    <li class="max-w-55 min-[1200px]:max-w-full">
                        <button type="button" @click="dropdown = !dropdown" class="nav-link nav-dropdown text-right">
                    ...
                    <li class="flex items-center text-right"><a href="{{ route('resources') }}" class="nav-link">Panduan Belajar</a></li>
                </ul>
    ...
    {{-- Dropdown --}}
    <div @click.outside="dropdown = false" x-cloak x-show="dropdown" class="z-[-1] relative"
        ...
        <div class="absolute hidden top-0 left-1/2 -translate-x-1/2 w-full bg-white after:absolute after:top-0 after:right-0 after:w-1/2 after:h-full after:bg-light-gray-100 lg:block">
            <div class="relative z-10 max-w-480 flex bg-white mx-auto lg:flex-col xl:flex-row">
                ...
                <div class="flex flex-col gap-4 bg-light-gray-100 p-10 w-full xl:max-w-117.5">
                    <button type="button" class="group relative rounded-2xl w-full aspect-video overflow-hidden cursor-pointer lg:max-w-117.5 xl:max-w-full 2xl:max-w-97.5 2xl:h-54.25">
                        <img class="transition-all duration-300 ease-in-out top-1/2 left-1/2 absolute size-14 -translate-x-1/2 -translate-y-1/2 group-hover:scale-110" src="{{ asset('images/icons/white-play.svg') }}">
