            ...
            <span class="relative z-0 inline-flex gap-x-2 h-10 rtl:flex-row-reverse">
                <span class="flex justify-center items-center">
                    ...
                    @if (!$paginator->onFirstPage())
                        <button type="button" class="cursor-pointer" wire:click="previousPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" data-analytics-name="sequence | Previous | {{ $currentPage - 1 }}" dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.after" aria-label="{{ __('pagination.previous') }}">
                            ...
                            @foreach ($element as $page => $url)
                                <div wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}" class="flex justify-center items-center w-6">
                                    @if ($page == $paginator->currentPage())
                                        <div aria-current="page" data-analytics-name="sequence | {{ $page }}" class="relative w-full text-center after:absolute after:left-0 after:-bottom-2 after:bg-mint-300 after:w-full after:h-1">
                                            {{ $page }}
                                        </div>
                                    @else
                                        <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" data-analytics-name="sequence | {{ $page }}" class="cursor-pointer w-full text-center" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                            {{ $page }}
                                        </button>
                                    @endif
                                </div>
                ...
                <span class="flex justify-center items-center">
                    ...
                    @if ($paginator->hasMorePages())
                        <button type="button" class="cursor-pointer" wire:click="nextPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" data-analytics-name="sequence | Next | {{ $currentPage + 1 }}" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.after" aria-label="{{ __('pagination.next') }}">
                            <svg width="18" height="32" viewBox="0 0 18 32" fill="none" xmlns="http://www.w3.org/2000/svg">
